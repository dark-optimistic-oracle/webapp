...
- Update `LOG.md` before declaring the work complete. Logging is not finished until the relevant `LOG.md` is staged and committed in this repository. Include it in the same commit as related fixes or documentation when practical; otherwise create a dedicated log commit.
...
- Push the commit containing `LOG.md` to the configured remote branch unless the user explicitly requests a local-only commit or the remote is unavailable. Verify the branch is synchronized afterward. If committing or pushing is blocked, report the exact uncommitted or unpushed state instead of treating the task as complete.
